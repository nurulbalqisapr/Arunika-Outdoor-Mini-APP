...
def add_rental(id_transaksi, userid, kode_barang, jumlah, lama_sewa, total_harga, status='active'):
    try:
        # Cek apakah record sudah ada (id_transaksi + kode_barang)
        existing = Rental.query.filter_by(id_transaksi=id_transaksi, kode_barang=kode_barang).first()
        
        if existing:
            # Jika ada, update datanya (Upsert)
            existing.jumlah = jumlah
            existing.lama_sewa = lama_sewa
...
def update_rental(id_transaksi, userid, kode_barang, jumlah, lama_sewa, total_harga, status):
    try:
        Rental.query.filter_by(id_transaksi=id_transaksi, kode_barang=kode_barang).update({
...
def delete_rental(id_transaksi, userid, kode_barang):
    try:
        Rental.query.filter_by(id_transaksi=id_transaksi, kode_barang=kode_barang).delete()
        db.session.commit()
    except Exception as e:
        db.session.rollback()
        raise e
