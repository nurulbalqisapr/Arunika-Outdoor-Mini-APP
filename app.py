...
import datetime
from Database import db, Rental # Import db from Database.py
...
def user_create_rental():
    ...
    if request.method == "POST":
        ...
        if confirm == "yes":
            ...
            # Cek apakah item ini sudah ada di rental aktif untuk update stok yang benar
            existing_active = next((t for t in active_rentals if t['kode_barang'] == kode_barang and t['status'] == 'active'), None)
            
            stok_sementara = barang['stok']
            if existing_active:
                # Jika item sudah ada, kita "kembalikan" stok lamanya dulu sebelum dikurangi stok baru
                stok_sementara += existing_active['jumlah']

            if stok_sementara < jumlah:
                return render_template("create_rental.html", error="⚠️ Sorry, Equipment is out of stock or insufficient.", info_user=info_user, active_rentals=active_rentals)

            add_rental(id_transaksi, userid, kode_barang, jumlah, lama_sewa, total_harga, "active")
            update_equipment(barang['kode'], barang['jenis'], barang['nama'], barang['harga'], stok_sementara - jumlah)
