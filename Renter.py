...
def buat_penyewaan(userid):
    ...
    while True:
        ...
        if pilih == "1":
            ...
            # CEK DUPLIKASI UNTUK UPDATE STOK
            existing_active = next((t for t in transaksi_user if t['kode_barang'] == kode_barang and t['status'] == 'active'), None)
            
            stok_sementara = barang_dipilih['stok']
            if existing_active:
                stok_sementara += existing_active['jumlah']

            if stok_sementara < jumlah:
                print(f"❌ Insufficient stock. Maximum available: {stok_sementara}")
                continue
            ...
            while True:
                ...
                if konfirmasi == "y":
                    ...
                    # Update stok di DB
                    update_equipment(barang_dipilih['kode'], barang_dipilih['jenis'], barang_dipilih['nama'], barang_dipilih['harga'], stok_sementara - jumlah)
